@extends('layouts.admin', ['heading' => 'Reschedule Booking'])
...
<div class="grid gap-6">
    ...
    <div x-data="{
        ...
        <form method="POST" action="{{ route('admin.bookings.reschedule', $booking) }}" class="rounded-2xl border border-gaz-border bg-gaz-card p-6">
            @csrf
            @method('PATCH')
            ...
            <h1 class="text-3xl font-black">Reschedule {{ $booking->booking_code }}</h1>
            <p class="mt-2 text-sm text-gaz-muted">{{ $booking->user->name }} - {{ $services }} - {{ $booking->capster->name }}</p>
            ...
            <div class="mt-8 flex justify-between gap-3">
                <x-secondary-button href="{{ route('admin.bookings.show', $booking) }}">Batal</x-secondary-button>
